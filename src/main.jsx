...
function ProjectItem({ item }) {
  const visual = <><div className="project-image"><ContentImage src={item.image} alt={item.title}/><div className="project-overlay"><div>{item.hover.map((line, index) => <span key={index} style={line.includes('不知道怎么形容') ? { whiteSpace: 'nowrap' } : undefined}>{line}</span>)}</div></div></div><p className="project-caption"><span>{item.label}</span>{item.subtitle && <span>{item.subtitle}</span>}</p></>
...
function Home() {
  ...
  useEffect(() => {
    ...
    const preload = index => new Promise(resolve => {
      ...
      image.onload = () => {
        ...
        decoded.finally(() => {
          if (active) setLoadedSlides(current => current.map((value, itemIndex) => itemIndex === index ? true : value))
          resolve()
        })
      }
  ...
  const slidesReady = loadedSlides.every(Boolean)
  ...
  useLayoutEffect(() => {
    ...
    const context = gsap.context(() => {
      ...
      root.querySelectorAll('.motion-section').forEach(section => {
        ...
        const imageFrames = isProjects ? section.querySelectorAll('.project-image img') : section.querySelectorAll('.about-visual')
        ...
        timeline
          ...
          .from(imageFrames, { yPercent: isProjects ? 0 : 7, scale: isProjects ? 1.1 : 1.035, duration: 1.22, stagger: isProjects ? 0.07 : 0.12, ease: 'power4.out', clearProps: 'transform' }, 0.34)
...
function Project2023() {
  return <main className="project-page project-2023">
    <a className="project-back" href="/#projects">← back to portfolio</a>
    ...
    <div className="project-images">
      <ContentImage src="/images/1.1.jpg" alt="A Seed Contains the Universe model detail" />
      <ContentImage src="/images/1.2.jpg" alt="A Seed Contains the Universe drawing" />
    </div>
...
function Project2024() {
  return <main className="project-page project-2024">
    ...
    <div className="project-images">
      <ContentImage src="/images/2.1.jpg" alt="Student dormitory design drawing one" />
      <ContentImage src="/images/2.2.jpg" alt="Student dormitory design drawing two" />
      <ContentImage src="/images/2.3.jpg" alt="Student dormitory design drawing three" />
      <ContentImage src="/images/2.4.jpg" alt="Student dormitory design model" />
    </div>
    <p className="project-description">文字介绍正在施工中......<ContentImage className="construction-mark" src="/images/under-construction.png" alt="Under construction" /></p>
  </main>
}
...
function Project2025() {
  return <main className="project-page project-2025">
    ...
    <div className="project-images">
      <ContentImage src="/images/3.1.jpg" alt="Small campus building design image one" />
      <ContentImage src="/images/3.2.jpg" alt="Small campus building design image two" />
      <ContentImage src="/images/3.3.jpg" alt="Small campus building design image three" />
      <ContentImage src="/images/3.4.jpg" alt="Small campus building design image four" />
    </div>
    <p className="project-description">文字介绍正在施工中......<ContentImage className="construction-mark" src="/images/under-construction.png" alt="Under construction" /></p>
  </main>
}
...
function Project2026() {
  return <main className="project-page project-2026">
    ...
    <div className="project-images">
      <ContentImage src="/images/4.1.jpg" alt="Screenshot Saved image one" />
      <ContentImage src="/images/4.2.jpg" alt="Screenshot Saved image two" />
      <ContentImage src="/images/4.3.jpg" alt="Screenshot Saved image three" />
      <ContentImage src="/images/4.4.jpg" alt="Screenshot Saved image four" />
      <ContentImage src="/images/4.5.jpg" alt="Screenshot Saved image five" />
      <ContentImage src="/images/4.6.jpg" alt="Screenshot Saved image six" />
    </div>
...
function ProjectVelux() {
  ...
  return <main className="project-page project-velux">
    ...
    <div className="project-images">
      <ContentImage src="/images/5.1.jpg" alt="Sun the Quilt image one" />
      <ContentImage src="/images/5.2.jpg" alt="Sun the Quilt image two" />
      <video src="/images/5.3.mp4" poster="/images/5.2.jpg" controls playsInline preload="none" aria-label="Sun the Quilt video" />
      <ContentImage src="/images/5.4.jpg" alt="Sun the Quilt image three" />
    </div>
...
function ProjectBridgeVillage() {
  return <main className="project-page project-bridge-village">
    ...
    <div className="project-images">
      <ContentImage src="/images/6.1.jpg" alt="Qiaoshan Village Community Activity Center Design image one" />
      <ContentImage src="/images/6.2.jpg" alt="Qiaoshan Village Community Activity Center Design image two" />
      <ContentImage src="/images/6.3.jpg" alt="Qiaoshan Village Community Activity Center Design image three" />
    </div>
    <p className="project-description">该项目正在施工中（2026.8）<ContentImage className="construction-mark" src="/images/under-construction.png" alt="Under construction" /></p>
  </main>
}
...
function ProjectInstallation() {
  return <main className="project-page project-installation">
    ...
    <div className="project-images">
      <ContentImage src="/images/7.1.jpg" alt="Ting installation image one" />
      <video src="/images/7.2.mp4" poster="/images/7.1.jpg" controls playsInline preload="none" aria-label="Ting installation video" />
      <ContentImage src="/images/7.3.jpg" alt="Ting installation image two" />
    </div>
...
function ProjectSnowFestival() {
  return <main className="project-page project-snow-festival">
    ...
    <div className="project-images">
      <ContentImage src="/images/8.1.jpg" alt="Snow Structure Festival image one" />
      <ContentImage src="/images/8.2.jpg" alt="Snow Structure Festival image two" />
    </div>
    <p className="project-description">... ...</p>
  </main>
}
...
function ProjectGallery({ project }) {
  ...
  const imageButton = (image, index) => <button className="gallery-image" key={image} onClick={() => setSelected(image)} aria-label={`Enlarge ${project.alt} image ${index + 1}`}><ContentImage src={`/images/${image}`} alt={`${project.alt} image ${index + 1}`} /></button>
  ...
  return <main className="project-page gallery-page">
    ...
    {selected && <div className="gallery-lightbox" role="dialog" aria-modal="true" aria-label="Enlarged image" onClick={close}><button className="lightbox-close" onClick={close} aria-label="Close enlarged image">×</button><ContentImage priority src={`/images/${selected}`} alt={`${project.alt} enlarged`} onClick={event => event.stopPropagation()} /></div>}
  </main>
}

function ProjectDetail({ project }) { return <main className="detail"><a className="back" href="/#projects">← Back to portfolio</a><div className="detail-copy"><p className="section-tag">&lt;Project&gt;</p><h1>{project.title}</h1><p>{project.label}</p></div><ContentImage src={project.image} alt={project.title}/><p className="detail-note">Project page in preparation. More drawings, process material and project text will be added here.</p></main> }
